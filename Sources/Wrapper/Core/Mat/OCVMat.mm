...
- (instancetype)initWithRows:(NSInteger)rows cols:(NSInteger)cols {
    return [self initWithRows:rows cols:cols type:OCVDepthTypeU8 channels:4];
}
...
- (instancetype)initWithData:(nonnull void *)data size:(OCVSize)size type:(OCVDepthType)type channels:(NSInteger)channels step:(size_t)step {
    // Convert type to OpenCV type
    int cvType = (int)CV_MAKETYPE(type, channels);
    return [super initWithMatInstance:new cv::Mat((int)size.height, (int)size.width, cvType, data, step)];
}
...
- (instancetype)initWithImage:(UIImage *)image {
    cv::Mat* mat = [self.class matFromImageRef:image.CGImage];
    return [self initWithMatInstance:mat];
}
...
- (instancetype)initWithCGSize:(CGSize)size {
    return [self initWithCGSize:size type:OCVDepthTypeU8 channels:4];
}
...
+ (cv::Mat*)matFromImageRef:(CGImageRef)imageRef {
    ...
    cv::Mat* cvMat = new cv::Mat((int)rows, (int)cols, CV_8UC4);
...
//    CVPixelBufferUnlockBaseAddress(buffer, kCVPixelBufferLock_ReadOnly);
    ...
    cv::Mat* cvMat = new cv::Mat((int)height, (int)width, format_opencv, bufferAddress, bytesPerRow);
...
//    NSData *data = [NSData dataWithBytes:cvMat.data length:cvMat.elemSize() * cvMat.total()];
    NSData *data = [NSData dataWithBytes:cvMat.data length:cvMat.step[0] * cvMat.rows];
